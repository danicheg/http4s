...
import org.http4s.metrics.prometheus.HttpMetricsNames._
...
/**
  ...
  */
final class HttpMetricsNames private (
  ...
  override def equals(obj: Any): Boolean =
    obj match {
      case metricsNames: HttpMetricsNames =>
  ...
  def withResponseDuration(responseDuration: String): Validated[HttpMetricsNames] =
    checkMetricName(responseDuration)
      .map(validatedName => copy(responseDuration = validatedName))

  def withActiveRequests(activeRequests: String): Validated[HttpMetricsNames] =
    checkMetricName(activeRequests)
      .map(validatedName => copy(activeRequests = validatedName))

  def withRequests(requests: String): Validated[HttpMetricsNames] =
    checkMetricName(requests)
      .map(validatedName => copy(requests = validatedName))

  def withAbnormalTerminations(abnormalTerminations: String): Validated[HttpMetricsNames] =
    checkMetricName(abnormalTerminations)
      .map(validatedName => copy(abnormalTerminations = validatedName))

  def withPrefix(prefix: String): Validated[HttpMetricsNames] =
    apply(
...
object HttpMetricsNames {
  ...
  ): Validated[HttpMetricsNames] = {
    (checkMetricName(responseDuration),
     checkMetricName(activeRequests),
     checkMetricName(requests),
     checkMetricName(abnormalTerminations))
      .mapN(new HttpMetricsNames(_, _, _, _))
  }
  ...
  def unsafeCreate(responseDuration: String,
                   activeRequests: String,
                   requests: String,
                   abnormalTerminations: String): HttpMetricsNames =
    new HttpMetricsNames(responseDuration = responseDuration,
                         activeRequests = activeRequests,
                         requests = requests,
                         abnormalTerminations = abnormalTerminations)

  val DefaultMetricsNames: HttpMetricsNames =
    new HttpMetricsNames(
