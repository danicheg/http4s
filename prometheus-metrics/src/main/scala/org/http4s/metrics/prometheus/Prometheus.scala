...
/** [[MetricsOps]] algebra capable of recording Prometheus metrics
  ...
  *
  * Also, you can specify a metrics suffix too with a [[org.http4s.metrics.prometheus.HttpMetricsNames]]:
  *
  * {prefix}_{suffix}{labels=classifier,method,phase} - Histogram
  *
  * Or you can specify fully custom names of the metrics with a [[org.http4s.metrics.prometheus.HttpMetricsNames]]:
  *
  * Labels --
  *
  * method: Enumeration
...
object Prometheus {
  ...
  def metricsOps[F[_]: Sync](
      ...
      metricsNames: HttpMetricsNames = DefaultSettings.metricsNames
  ...
  private def createMetricsCollection[F[_]: Sync](
    ...
    val metricsNamesWithPrefix = prefix match {
      case None =>
        Resource.pure[F, HttpMetricsNames](metricsNames)
